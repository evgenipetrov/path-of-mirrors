...
export function BuildDisplay({ build }: BuildDisplayProps) {
  ...
  const { data: statDefinitions, isLoading: isLoadingStats } =
    useStatDefinitions(game as 'poe1' | 'poe2')

  // Initialize weights with all canonical stats from backend
  const defaultWeights = useMemo<Record<string, StatWeight>>(() => {
  ...
  const [weights, setWeights] =
    useState<Record<string, StatWeight>>(defaultWeights)

  const [isAddStatDialogOpen, setIsAddStatDialogOpen] = useState(false)

  // Update weights when default weights change (stat definitions loaded)
  useEffect(() => {
  ...
  const toPercent = (value?: number) =>
    value === undefined
      ? undefined
      : `${Math.round(value >= 10 ? value : value * 100)}%`
  ...
  const onWeightChange = (
    key: string,
    field: 'percentWeight' | 'flatWeight' | 'gainType',
    value: number | string
  ) => {
    setWeights((prev) => ({
      ...prev,
  ...
  const defaultHiddenStats = useMemo<Set<string>>(() => {
  ...
  const [hiddenStats, setHiddenStats] =
    useState<Set<string>>(defaultHiddenStats)

  const [isStatsCollapsed, setIsStatsCollapsed] = useState(false)

  // Update hidden stats when default hidden stats change (stat definitions loaded)
  useEffect(() => {
    setHiddenStats(defaultHiddenStats)
  }, [defaultHiddenStats])
  ...
  const availableStats =
    statDefinitions?.stats.filter((stat) => hiddenStats.has(stat.key)) ?? []

  // Group available stats by category
  const statsByCategory = availableStats.reduce(
    (acc, stat) => {
        ...
        <CardContent className='space-y-6'>
          ...
          <div className='flex flex-wrap items-center gap-2 text-sm'>
            ...
            {derived?.charges && (
              <>
                <Separator orientation='vertical' className='h-4' />
                <span className='text-muted-foreground font-medium'>
                  {derived.charges.endurance ?? 0}/{derived.charges.frenzy ?? 0}
                  /{derived.charges.power ?? 0}
                </span>
                <span className='text-muted-foreground text-xs'>E/F/P</span>
              </>
            )}
          </div>
          ...
          <div className='flex items-center justify-between'>
            ...
            <div className='flex gap-2'>
              <Dialog
                open={isAddStatDialogOpen}
                onOpenChange={setIsAddStatDialogOpen}
              >
                ...
                <DialogContent className='max-h-[80vh] max-w-md overflow-y-auto'>
                  <DialogHeader>
                    ...
                    <DialogDescription>
                      Select a stat to add to your weights table. These are
                      canonical stats from the backend.
                    </DialogDescription>
                  </DialogHeader>
                  <div className='space-y-4 py-4'>
                    {Object.entries(statsByCategory).map(
                      ([category, stats]) => (
              ...
              <TableBody>
                {[
                  isHidden('life')
                    ...
                    : {
                        label: 'Life',
                        value: derived?.life ?? build.life,
                        numericValue: derived?.life ?? build.life,
                        accent: 'text-red-500',
                        weightKey: 'life',
                      },
                  isHidden('energy_shield')
                    ? undefined
                    : {
                        label: 'Energy Shield',
                        value: derived?.es ?? build.energy_shield,
                        numericValue: derived?.es ?? build.energy_shield,
                        weightKey: 'energy_shield',
                      },
                  isHidden('mana')
                    ? undefined
                    : {
                        label: 'Mana',
                        value: derived?.mana ?? build.mana,
                        numericValue: derived?.mana ?? build.mana,
                        weightKey: 'mana',
                      },
                  isHidden('armour')
                    ? undefined
                    : {
                        label: 'Armour',
                        value: derived?.armour ?? build.armour,
                        numericValue: derived?.armour ?? build.armour,
                        weightKey: 'armour',
                      },
                  isHidden('evasion')
                    ? undefined
                    : {
                        label: 'Evasion',
                        value: derived?.eva ?? build.evasion,
                        numericValue: derived?.eva ?? build.evasion,
                        weightKey: 'evasion',
                      },
                  derived?.block?.attack !== undefined && !isHidden('block')
                    ? {
                        label: 'Block',
                        value: toPercent(derived.block.attack),
                        numericValue: derived.block.attack,
                        weightKey: 'block',
                      }
                    : undefined,
                  derived?.block?.spell !== undefined &&
                  !isHidden('spell_block')
                    ? {
                        label: 'Spell Block',
                        value: toPercent(derived.block.spell),
                        numericValue: derived.block.spell,
                        weightKey: 'spell_block',
                      }
                    : undefined,
                  derived?.block?.suppression !== undefined &&
                  !isHidden('spell_suppression')
                    ? {
                        label: 'Spell Suppression',
                        value: toPercent(derived.block.suppression),
                        numericValue: derived.block.suppression,
                        weightKey: 'spell_suppression',
                      }
                    : undefined,
                  derived?.res?.fire !== undefined && !isHidden('fire_res')
                    ? {
                        label: 'Fire Res',
                        value: `${derived.res.fire}%`,
                        numericValue: derived.res.fire,
                        weightKey: 'fire_res',
                      }
                    : undefined,
                  derived?.res?.cold !== undefined && !isHidden('cold_res')
                    ? {
                        label: 'Cold Res',
                        value: `${derived.res.cold}%`,
                        numericValue: derived.res.cold,
                        weightKey: 'cold_res',
                      }
                    : undefined,
                  derived?.res?.lightning !== undefined &&
                  !isHidden('lightning_res')
                    ? {
                        label: 'Lightning Res',
                        value: `${derived.res.lightning}%`,
                        numericValue: derived.res.lightning,
                        weightKey: 'lightning_res',
                      }
                    : undefined,
                  derived?.res?.chaos !== undefined && !isHidden('chaos_res')
                    ? {
                        label: 'Chaos Res',
                        value: `${derived.res.chaos}%`,
                        numericValue: derived.res.chaos,
                        weightKey: 'chaos_res',
                      }
                    : undefined,
                  isHidden('dps')
                  ...
                  isHidden('ehp')
                    ...
                    : {
                        label: 'Effective HP',
                        value: derived?.ehp,
                        numericValue: derived?.ehp,
                        weightKey: 'ehp',
                      },
                  derived?.movement_speed !== undefined &&
                  !isHidden('movement_speed')
                    ? {
                        label: 'Movement Speed',
                        value: toPercent(derived.movement_speed),
                        numericValue: derived.movement_speed,
                        weightKey: 'movement_speed',
                      }
                    : undefined,
                  derived?.max_hit &&
                  [
                    ...
                    ? isHidden('max_hit')
                      ...
                      : {
                          label: 'Max Hit',
                          value: [
                            derived.max_hit.physical !== undefined &&
                              `Phys ${Math.round(derived.max_hit.physical).toLocaleString()}`,
                            derived.max_hit.fire !== undefined &&
                              `Fire ${Math.round(derived.max_hit.fire).toLocaleString()}`,
                            derived.max_hit.cold !== undefined &&
                              `Cold ${Math.round(derived.max_hit.cold).toLocaleString()}`,
                            derived.max_hit.lightning !== undefined &&
                              `Lightning ${Math.round(derived.max_hit.lightning).toLocaleString()}`,
                            derived.max_hit.chaos !== undefined &&
                              `Chaos ${Math.round(derived.max_hit.chaos).toLocaleString()}`,
                          ]
                            .filter(Boolean)
                            .join(' · '),
                  ...
                  .map((stat, idx) => {
                    const weight = stat.weightKey
                      ? getWeight(stat.weightKey)
                      : null
                    return (
                      <TableRow key={`stat-${idx}-${stat.label}`}>
                        ...
                        <TableCell
                          className={`px-3 text-right ${stat.accent ?? ''}`}
                        >
                        ...
                        <TableCell className='px-3'>
                          {weight && (
                            <Input
                              ...
                              step='0.05'
                              min='0'
                              className='h-7 w-full px-2 text-right text-xs'
                        ...
                        <TableCell className='px-3'>
                          {weight && (
                            <select
                              className='border-input bg-background hover:bg-accent h-7 w-full cursor-pointer rounded-md border px-2 text-xs'
                ...
                {Object.keys(weights)
                  ...
                  .map((statKey) => {
                    ...
                    const statDef = statDefinitions?.stats.find(
                      (s) => s.key === statKey
                    )
                    const displayName =
                      statDef?.display_name ||
                      statKey
                        .replace(/_/g, ' ')
